...
public class TracingConfiguration {
	...
	@Value("${jaeger.reporter.minDurationMicroseconds:2000}")
	private int minDurationMicroseconds;
	...
	public void init() {
		...
		if (scheme.equals("udp")) {
			...
			if (!AddressAvailabilityCondition.check(s, 2000)) {
				log.warn("Skip tracing with jaeger {}", s);
				return;
			}
		...
		if (production) {
			...
			reporter = new DelegatingReporter(reporter, span -> span.getDuration() >= minDurationMicroseconds);
		} else {
		...
		log.info("Register tracer with jaeger {}", uri);
	}
