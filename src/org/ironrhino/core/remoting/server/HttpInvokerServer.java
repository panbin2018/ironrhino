...
public class HttpInvokerServer implements HttpRequestHandler {
	...
	public void handleRequest(HttpServletRequest request, HttpServletResponse response)
		...
		ThreadLocal<Object> scopeHolder = Tracing.isOpentracingPresent() ? new ThreadLocal<Object>() : null;
		invoke(request, response, req -> {
			if (Tracing.isOpentracingPresent()) {
				...
				scopeHolder.set(tracer.scopeManager().activate(span, true));
			}
			...
			if (Tracing.isOpentracingPresent()) {
				Scope scope = (Scope) scopeHolder.get();
				if (scope != null) {
					scope.close();
					scopeHolder.remove();
				}
